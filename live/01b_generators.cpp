...
struct DecodeAllAndApply<std::tuple<T...>> {
    ...
    auto operator()(CmdBlock& cmd_block, F&& f) {
        return CallWithSequentialEvaluation<F, T...> { f, ReadEntry<T>(cmd_block)... }.get();
    }
};
...
int main() {
    ...
    DecodeAllAndApply<CmdStuff::request_list>{}(cmd_block, DoStuff); // (S)PI
}
